...
class AccountFrFecUnique(models.TransientModel):
    ...
    def generate_fec(self):
        ...
        # If the 'Unique export' is not checked, the function keeps its basic operation
        if not self.unique_export:
        ...
        # Check if the query results are empty, dont generate the FEC file
        if not query_results:
            raise UserError(_('The file is empty, all the entries have already been exported'))
        ...
        # Mark as exported all accounting entries that have been exported if is not a test file
